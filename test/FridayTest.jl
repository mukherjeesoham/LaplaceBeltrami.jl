...
# NEW: New projection operators that use the metric to raise the index, and then
# project on a vector basis.
...
# This operator raises the index using the physical metric
function modal_to_nodal_vector_op_raised_index_with_h(S::SphericalHarmonics{T})::Array{Complex{T}, 2} where {T}
    lmax, n = S.lmax, S.N
    A = zeros(Complex{T}, 2*(n*(2*n)), (lmax)^2 + 2*(lmax)+1)
    for index in CartesianIndices(A)
        (i,j,a) = split3(index.I[1], n)
        (l,m)   = split(index.I[2])
        (θ,ϕ)   = collocation(S,i,j)
        A[index] = hinv(a,1,θ,ϕ)*GradSH(1, l, m, θ, ϕ) + hinv(a,2,θ,ϕ)*GradSH(2, l, m, θ, ϕ)
    end
    return A
end

function nodal_to_modal_vector_op_raised_index_with_h(S::SphericalHarmonics{T})::Array{Complex{T}, 2} where {T}
    return pinv(modal_to_nodal_vector_op_raised_index_with_h(S))
end

SH = SphericalHarmonics(30)
V̄  = nodal_to_modal_vector_op(SH) 
V̄new = nodal_to_modal_vector_op_raised_index(SH) 
V̄brandnew = nodal_to_modal_vector_op_raised_index_with_h(SH) 

# TODO: Project a vector field on the sphere and compare the pseudo-inverse
# with numerical integration. Gaussian product quadrature converges very slowly;
# not practical.
...
# Compute the error of a vector projection with Mathematica
# XXX: The integrals are sufficiently accurate
...
if true
    ...
    semilogy(max_coefficent_for_each_l(SH, V̄new*(H*∇nf)), "g-o")
    semilogy(max_coefficent_for_each_l(SH, V̄brandnew*(H*∇nf)), "k-o")
    show()
